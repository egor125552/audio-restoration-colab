...
def build_model_information(model_id: str) -> str:
    ...
    if model.backend != "stems":
        cache_note = ""
    elif model_id == "stems_instrumental_studio":
        cache_note = (
            "\n\n**Кэш и память:** веса скачиваются один раз и остаются на "
            "диске. Три модели запускаются по очереди. Перед загрузкой следующей "
            "предыдущая явно освобождается из памяти, чтобы уменьшить пик VRAM."
        )
    elif model.ensemble_preset:
        cache_note = (
            "\n\n**Кэш:** веса ансамбля скачиваются один раз и остаются на "
            "диске. Во время обработки модели ансамбля проходят по очереди."
        )
    else:
        cache_note = (
            "\n\n**Кэш:** веса скачиваются один раз; повторный запуск той же "
            "одиночной модели использует уже загруженную модель, пока работает "
            "Colab."
        )
    return (
        f"## {model.title}\n\n"
